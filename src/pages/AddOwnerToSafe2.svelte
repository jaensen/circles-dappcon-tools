<script lang="ts">
    ...
    async function addOwnerToSafe() {
        const store = await createAddOwnerStore($web3, $connectedWalletAddress, mnemonicToEntropy(mnemonicPhrase), selectedSafe.safeAddress);
        return store;
    }
</script>
