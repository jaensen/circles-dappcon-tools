 ...
 */
export function createLiveSearchStore(throttleInterval, performLiveSearch, defaultValue) {
    let buffer = undefined;
    let executing = false;
    const { subscribe, set } = writable(defaultValue);
    async function executeSearch(searchArg) {
        executing = true;
    ...
    async function search(searchArg) {
        buffer = searchArg;
        if (!executing) {
            const nextArg = buffer;
            buffer = undefined;
...
//# sourceMappingURL=createLiveSearchStore.js.map